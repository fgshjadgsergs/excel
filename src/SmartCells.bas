...
Private Sub ПройтиПоТексту(ByVal doc As Document, ByVal problems As Collection)
    ...
    With rng.Find
        ...
        .Format = False
        .MatchWildcards = True
        ' Ловим ЛЮБОЙ текст в фигурных скобках (без вложенных скобок и
        ' переносов абзаца): сложные наборы символов в wildcard капризны
        ' (Word требует строгий порядок кодов, кириллица ломает шаблон),
        ' поэтому вся фильтрация «метка/не метка» — ниже, в VBA
        ' (ПохожеНаМетку). Имя листа перед «!» допускается: {Лист!A5}
        .Text = "\{[!^13{}]@\}"
